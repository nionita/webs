# -*- coding: utf-8 -*-
"""
Created on Mon Apr  2 12:17:30 2018
...
from app.forms import LoginForm, RegistrationForm, ProfileEditForm, PostForm
from app.forms import ResetPwdReqForm, ResetPasswordForm
...
def explore():
    ...
    # Reuse the index.html template (but with no post form)
...
def reset_password_request():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('login'))
    return render_template('reset_pwd_req.html', title=_('Reset Password'), form=form)
...
def follow(username):
    ...
    if user == current_user:
        ...
        db.session.commit()
        flash(_('Now following user %(username)s', username=username))
...
def unfollow(username):
    ...
    if user == current_user:
        ...
        db.session.commit()
        flash(_('User %(username)s not followed anymore', username=username))
    return redirect(url_for('user', username=username))
